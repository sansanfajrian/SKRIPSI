...
const ViewAllBacklogs = () => {
  ...
  const retrieveAllBacklogs = () => {
    request({
      url: API_BASE_URL + "/api/backlog/fetch",
      method: "GET",
    })
      .then((response) => {
  ...
  const editBacklog = (backlogId) => {
    navigate(`/user/admin/backlog/edit/${backlogId}`);
  };

  const handleDelete = (backlogId) => {
    setDialogOpen(true);
    setDeleteBacklogId(backlogId);
  };
  
  const handleConfirm = () => {
    setDialogOpen(false);
    deleteBacklog(deleteBacklogId)
      .then(() => {
        toast.warning("Backlog deleted successfully", {
          position: "top-center",
          autoClose: 1000,
  ...
  const deleteBacklog = (backlogId) => {
    return request({
      url: API_BASE_URL + "/api/backlog/delete/" + backlogId,
      method: "DELETE",
    })
      .then((response) => {
        console.log(response);
        setAllBacklogs(allBacklogs.filter(backlog => backlog.backlogId !== backlogId));
      })
      .catch((error) => {
        console.log(error);
      });
                        ...
                        {records.map((backlog, i) => (
                          <tr key={i}>
                            ...
                            <td className="text-center">
                                <div>
                                    <button
                                    onClick={() => editBacklog(backlog.backlogId)}
                                    ...
                                    <button
                                    onClick={() => handleDelete(backlog.backlogId)}
...
export default ViewAllBacklogs;
